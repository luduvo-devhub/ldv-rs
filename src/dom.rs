...
impl Default for Components {
    fn default() -> Components {
        Components {
            ...
            size: Some(Vec3 { x: 1.0, y: 1.0, z: 1.0 }),
            name: None,

            color: Some(Color3 { r: 1.0, g: 1.0, b: 1.0 }),
...
impl Dom {
    pub fn from_file(file: &File) -> Self {
        ...
        for record in &file.records {
            ...
            match &record.entries {
                RecordEntries::Vec3(entries) => {
                    for e in entries {
                        ...
                        match record.component_type {
                            ComponentType::Position => ent.components.position = Some(e.value.clone()),
                ...
                RecordEntries::Lighting(_) | RecordEntries::PointLight(_) | RecordEntries::SpotLight(_) => {}
        ...
        // collect roots
        let root_entities = entities.values().filter(|e| e.parent.is_none()).map(|e| e.id).collect();
    ...
    pub fn to_file(&self) -> Result<File, DecodeError> {
        ...
        for entity in self.entities.values().filter(|e| e.is_real) {
            ...
            if !missing.is_empty() {
                missing_fields.push(format!("Entity {} missing: {}", entity.id, missing.join(", ")));
            }
        }
